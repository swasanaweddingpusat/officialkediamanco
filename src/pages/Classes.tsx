...
const Classes = () => {
  ...
  const filteredPrograms = selectedCategory ? activePrograms.filter(p => p.category === selectedCategory) : activePrograms;
  return <Layout>
      {/* Hero */}
      ...
      {/* Categories */}
      {categories.length > 0 && <section className="py-8 border-b border-border">
          <div className="container mx-auto px-4">
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {filteredPrograms.map((program, index) => <motion.div key={program.id} initial={{
            opacity: 0,
            y: 30
          }} whileInView={{
            opacity: 1,
            ...
            delay: index * 0.05
          }} onClick={() => navigate(`/classes/${program.id}`)} className="group relative overflow-hidden rounded-2xl aspect-[4/5] cursor-pointer">
